...
The Apple system font stack anchors this adaptation, ranging from a 64px hero display down to a 12px caption. Use `-apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro Display", "Helvetica Neue", Arial, sans-serif` instead of Meta's proprietary Optimistic face. Below 768px the system collapses cleanly: hero stacks, pill nav becomes a hamburger, three-up feature grids flatten to a single column, and product configurators drop their right-rail summary into a sticky bottom bar.
...
- Apple system/SF Pro as the universal display + body face; do not import Inter or Optimistic
...
### Font Family
Use the Apple system stack: `-apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro Display", "Helvetica Neue", Arial, sans-serif`. It replaces Meta's proprietary Optimistic face in this project while preserving the same scale, white-space discipline, and rounded component language. Display and headings use 600 instead of 500 for SF Pro's optical balance; editorial subheads use 400 instead of 300.
...
|---|---|---|---|---|---|---|
| `{typography.hero-display}` | 64px | 600 | 1.16 | 0 | normal | Homepage hero ("Get 25% off…", category opener) |
| `{typography.display-lg}` | 48px | 600 | 1.17 | 0 | normal | Section-opener display ("Made for prescriptions. Built for comfort.") |
| `{typography.heading-lg}` | 36px | 600 | 1.28 | 0 | normal | Subsection headlines ("Why buy from Meta", "Tech specs") |
| `{typography.heading-md}` | 28px | 400 | 1.21 | 0 | normal | Editorial subheads in lighter weight ("Look forward", "Built for prescriptions") |
| `{typography.heading-sm}` | 24px | 600 | 1.25 | 0 | normal | Card titles, feature-tile headers |
...
### Principles
- Negative letter-spacing on body roles (`-0.14px` to `-0.16px`) tightens the type fractionally; keep it subtle with SF Pro.
...
- Do not use proprietary Meta OpenType requirements here; rely on the Apple system stack and the documented weights.
...
- Use the Apple system stack for every heading/body role; do not import Optimistic, Inter, or another display face unless the design direction changes.
