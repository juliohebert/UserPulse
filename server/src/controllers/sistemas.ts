...
export async function criar(req: Request, res: Response) {
  try {
    ...
    const criado = await prisma.sistema.create({
      data: {
  ...
  try {
    ...
    const [atualizado] = await prisma.$transaction([
      prisma.sistema.update({
        where: { id },
        data: { nome, slug, identificador, descricao, url_base: urlBase, ativo: Boolean(ativo) },
        include: { _count: { select: { telas: true, aparencias: true } } },
      }),
...
export async function remover(req: Request, res: Response) {
  try {
    ...
    await prisma.sistema.update({ where: { id }, data: { ativo: false } })
    res.status(204).send()
  } catch {
