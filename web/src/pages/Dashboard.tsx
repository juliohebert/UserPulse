  ...
  return (
    <div className="min-h-[164px] bg-surface rounded-3xl border border-outline-variant p-6 flex flex-col">
      <div className="flex min-h-10 items-center gap-2.5">
        <span
          className={`w-8 h-8 rounded-2xl flex items-center justify-center shrink-0 ${iconBg} ${iconColor}`}
        >
          <span className="material-symbols-outlined text-[18px]">{icon}</span>
        </span>
        <p className="text-label-md font-medium text-on-surface-variant leading-tight">
          {label}
        </p>
      </div>
      <div className="mt-4 flex flex-1 flex-col">
        <p className="text-headline-lg font-semibold text-on-surface leading-none">
          {value}
        </p>
        {hint ? (
          <p
            className={`mt-auto min-h-5 text-[13px] font-medium leading-5 ${hintColor ?? "text-outline"}`}
          >
            {hint}
          </p>
        ) : (
...
function AcaoRapida({
  ...
  return (
    ...
    >
      <span
        className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${iconBg} ${iconColor}`}
      >
...
function InsightCard({
  ...
  return (
    <div className="flex items-start gap-3 p-5 rounded-3xl bg-surface border border-outline-variant">
      <span
        className={`w-9 h-9 rounded-2xl flex items-center justify-center shrink-0 ${iconBg} ${iconColor}`}
      >
        <span className="material-symbols-outlined text-[18px]">{icon}</span>
      </span>
      <div className="min-w-0 flex-1">
        ...
        <button
          onClick={onCta}
          className="text-label-md font-bold text-primary hover:underline mt-1.5"
        >
          {ctaLabel}
        </button>
      </div>
  ...
  return (
    <div className="bg-surface rounded-3xl border border-outline-variant p-8 sm:p-12 text-center">
      ...
      <p className="text-body-md text-on-surface-variant max-w-md mx-auto mb-6">
        Ainda não há campanhas nem tours guiados por aqui. Crie o primeiro
        conteúdo para começar a coletar feedback e guiar seus usuários dentro do
        produto.
      </p>
...
function TrialWelcomeModal({
  ...
  const limites = plano
    ? [
        {
          icon: "campaign",
          valor: fmtLimiteModal(plano.limite_campanhas_ativas),
          label: plano.limite_campanhas_ativas === 1 ? "Campanha" : "Campanhas",
        },
        {
          icon: "map",
          valor: fmtLimiteModal(plano.limite_tours_ativos),
          label:
            plano.limite_tours_ativos === 1 ? "Tour Guiado" : "Tours Guiados",
        },
        {
          icon: "route",
          valor: fmtLimiteModal(plano.limite_jornadas_ativas),
          label: plano.limite_jornadas_ativas === 1 ? "Jornada" : "Jornadas",
        },
      ]
    : [];
      ...
      <div className="bg-surface rounded-2xl shadow-xl w-full max-w-md overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 text-center">
          <h3
            ...
            className="text-headline-md font-bold text-on-surface mb-2"
          >
            ...
            <div className="bg-surface-container-low rounded-xl border border-outline-variant/40 p-4 mb-5 text-left">
              ...
              <div className="grid grid-cols-3 gap-2">
                {limites.map((l) => (
                  <div
                    key={l.label}
                    className="text-center bg-surface rounded-lg border border-outline-variant/30 py-3 px-1"
                  >
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button fullWidthMobile onClick={onComecar} className="sm:flex-1">
              Começar a explorar
            </Button>
          </div>
...
export function Dashboard() {
  ...
  const [campanhaInativar, setCampanhaInativar] = useState<Campanha | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      /* localStorage indisponível (modo privado, etc.) — nunca bloqueia a tela por isso */
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    user?.id,
    user?.tenant.id,
    user?.tenant.trial_fim,
    user?.tenant.situacao_comercial,
  ]);
    ...
    if (user) {
      try {
        localStorage.setItem(
          trialModalKey(user.tenant.id, user.id, user.tenant.trial_fim),
          "1",
        );
      } catch {}
    }
  ...
  const totalFeedbacks = campanhas.reduce(
    (s, c) => s + (c._count?.feedbacks ?? 0),
    0,
  );
  const recentes = campanhas.slice(0, 5);
  const maxFeedbacks = Math.max(
    1,
    ...recentes.map((c) => c._count?.feedbacks ?? 0),
  );
  ...
  const campanhasSemFeedback = campanhas.filter(
    (c) => getStatus(c) === "ativa" && (c._count?.feedbacks ?? 0) === 0,
  );
  ...
  if (total > 0 && totalFeedbacks === 0 && campanhasSemFeedback.length === 0) {
    insights.push({
      ...
      description:
        "Confira se as campanhas estão sendo exibidas corretamente para os usuários.",
      ctaLabel: "Ver campanhas",
      onCta: () => navigate("/campanhas"),
    });
  }
  ...
  const isEmpty =
    !loading && !error && toursDataReady && total === 0 && totalTours === 0;
    ...
    <section className="px-4 lg:px-margin-desktop py-8">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-primary p-8 sm:p-10 text-white mb-8">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <div>
            <p className="text-label-md font-bold uppercase tracking-widest opacity-75 mb-2">
              Visão geral
            </p>
            <h2 className="text-headline-lg sm:text-display-lg font-semibold leading-tight mb-2">
              Bem-vindo de volta
            </h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
            ...
            <KpiCard
              ...
              hint={
                toursLoading
                  ? undefined
                  : `${totalTours} criado${totalTours === 1 ? "" : "s"}`
              }
              hintColor="text-tertiary"
            />
            <KpiCard
            ...
            <>
                ...
                {insightsVisiveis.length === 0 ? (
                  <div className="flex items-center gap-3 p-6 rounded-3xl bg-white border border-outline-variant">
                    ...
                    <p className="text-body-md text-on-surface font-medium">
                      Tudo certo por aqui. Continue acompanhando a adoção dos
                      usuários.
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {insightsVisiveis.map((ins) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                ...
                <div className="lg:col-span-8 bg-surface rounded-3xl border border-outline-variant overflow-hidden">
                  ...
                  {recentes.length === 0 ? (
                    <p className="text-body-md text-outline px-5 pb-5">
                      ...
                      <table className="w-full text-left">
                        <thead>
                          <tr className="border-y border-outline-variant/30">
                            <th className="px-5 py-2.5 text-label-md text-on-surface-variant font-semibold">
                              Título
                            </th>
                            <th className="px-5 py-2.5 text-label-md text-on-surface-variant font-semibold text-center">
                              Status
                            </th>
                            <th className="px-5 py-2.5 text-label-md text-on-surface-variant font-semibold">
                              Tipo
                            ...
                            <th className="px-5 py-2.5 text-label-md text-on-surface-variant font-semibold text-right">
                              Ações
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-outline-variant/20">
                          {recentes.map((c) => {
                            const pct = Math.round(
                              ((c._count?.feedbacks ?? 0) / maxFeedbacks) * 100,
                            );
                            return (
                              <tr
                                key={c.id}
                ...
                <div className="lg:col-span-4 space-y-4">
                  ...
                  <div className="bg-surface-container-lowest p-5 rounded-2xl shadow-sm border border-outline-variant/30">
                    ...
                    {toursLoading ? (
                      ...
                      <p className="text-body-md text-on-surface-variant py-2">
                        Não foi possível carregar os tours guiados agora.
                      </p>
                    ) : totalTours === 0 ? (
                      <p className="text-body-md text-on-surface-variant py-2">
                        Nenhum tour guiado criado ainda.
                      </p>
                    ) : (
                      <>
                            ...
                            >
                              <div className="min-w-0">
                                ...
                                <p className="text-[11px] text-on-surface-variant">
                                  {t._count?.passos ?? 0} passo
                                  {(t._count?.passos ?? 0) === 1 ? "" : "s"}
                                </p>
                              </div>
                  ...
                  {/* Activity feed */}
                  <div className="bg-surface-container-lowest p-5 rounded-2xl shadow-sm border border-outline-variant/30">
                    <h3 className="text-title-lg font-bold text-on-surface mb-4">
                      Atividade Recente
                    </h3>
                    {atividades.length === 0 ? (
                      <p className="text-body-md text-on-surface-variant py-2">
                      ...
                      <div>
                        {atividades.map((a, i) => (
                          ...
                          >
                            ...
                            >
                              <span
                                className={`material-symbols-outlined text-[18px] ${a.tipo === "campanha" ? "text-primary" : "text-secondary"}`}
                              >
