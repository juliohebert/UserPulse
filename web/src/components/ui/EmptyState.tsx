  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-8 text-center">
      <div className="w-16 h-16 rounded-2xl bg-surface-container-low flex items-center justify-center mb-4">
        <span className="material-symbols-outlined text-[32px] text-steel">{icon}</span>
      </div>
      <h3 className="text-title-lg font-semibold text-ink-deep mb-2">{title}</h3>
      {description && <p className="text-body-md text-charcoal mb-6 max-w-sm">{description}</p>}
      {action}
    </div>
  )
}
